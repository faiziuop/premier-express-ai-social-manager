...
function normalizeDraft(draft,related,category){
 ...
 const ensureCoreSection=(test,heading,content,bullets=[])=>{if(draft.sections.some(section=>test.test(String(section.heading||""))))return;const whyIndex=draft.sections.findIndex(section=>/why choose/i.test(String(section.heading||"")));draft.sections.splice(whyIndex>=0?whyIndex:draft.sections.length,0,{level:"H2",heading,content,bullets})};
 ensureCoreSection(/highlight/i,focusPhrase+" Highlights","Key experience details are confirmed from the selected product option before booking.");
 ensureCoreSection(/inclusion|included/i,focusPhrase+" Inclusions","Only the services and access expressly listed in the selected booking option and confirmation are included.");
 ensureCoreSection(/exclusion|not included|additional charge/i,focusPhrase+" Exclusions and Additional Charges","Anything not expressly listed as included is excluded unless it is confirmed in the selected booking details.");
 ...
 for(const section of draft.sections){const heading=String(section.heading||"");if(/overview/i.test(heading))section.heading=focusPhrase+" Overview";else if(/what to expect/i.test(heading))section.heading="What to Expect from "+(relatedPhrases[0]||focusPhrase);else if(/highlight/i.test(heading)&&!/inclusion|exclusion/i.test(heading))section.heading=focusPhrase+" Highlights";else if(/inclusion|included/i.test(heading)&&!/exclusion/i.test(heading))section.heading=focusPhrase+" Inclusions";else if(/exclusion|not included/i.test(heading))section.heading=focusPhrase+" Exclusions and Additional Charges";else if(/timing|duration|service availability|operating hour/i.test(heading))section.heading=focusPhrase+" Timings and Duration";else if(/booking|cancellation|refund|policy/i.test(heading))section.heading=focusPhrase+" Booking, Cancellation and Refund Policy";else if(/why choose/i.test(heading))section.heading="Why Choose Premier Express Tourism for "+focusPhrase;}
 const insertBeforeWhy=section=>{const whyIndex=draft.sections.findIndex(row=>/why choose/i.test(String(row.heading||"")));draft.sections.splice(whyIndex>=0?whyIndex:draft.sections.length,0,section)};
 const finalHeads=()=>draft.sections.map(section=>String(section.heading||"").toLowerCase());
 if(!finalHeads().some(heading=>heading.includes("highlight")&&!heading.includes("inclusion")&&!heading.includes("exclusion")))insertBeforeWhy({level:"H2",heading:focusPhrase+" Highlights",content:"The experience highlights are presented from the confirmed product scope and selected booking option."});
 if(!finalHeads().some(heading=>heading.includes("inclusion")&&!heading.includes("exclusion")))insertBeforeWhy({level:"H2",heading:focusPhrase+" Inclusions",content:"Only services and access expressly listed in the selected booking option and confirmation are included."});
 if(!finalHeads().some(heading=>heading.includes("exclusion")||heading.includes("not included")))insertBeforeWhy({level:"H2",heading:focusPhrase+" Exclusions and Additional Charges",content:"Anything not expressly listed as included is excluded unless confirmed in the selected booking details."});
 let whySection=draft.sections.find(section=>/why choose/i.test(String(section.heading||"")));if(!whySection){whySection={level:"H2",heading:"Why Choose Premier Express Tourism for "+focusPhrase,bullets:[]};draft.sections.push(whySection)}
 whySection.bullets=Array.isArray(whySection.bullets)?whySection.bullets.map(cleanText).filter(Boolean).slice(0,6):[];
 const whyFallbacks=[
  "Product-specific guidance for "+focusPhrase+" is organised around the selected access option, timing and confirmed visitor conditions.",
  "The booking information separates confirmed inclusions, exclusions and possible additional charges for clearer review before payment.",
  "Relevant entry, meeting and visitor details are presented together so the selected option can be checked before confirmation.",
  "Related experiences are selected from the live Premier Express Tourism catalogue to support practical itinerary planning.",
  "Booking conditions can be clarified against the exact product option before payment when further confirmation is required."
 ];for(const item of whyFallbacks){if(whySection.bullets.length>=5)break;if(!whySection.bullets.some(existing=>String(existing).toLowerCase()===item.toLowerCase()))whySection.bullets.push(item)}
 ...
 let timingSection=draft.sections.find(section=>/timings?|durations?|schedule|service availability|operating hours?/i.test(String(section.heading||"")));
 if(!timingSection){timingSection={level:"H2",heading:focusPhrase+" Timings and Duration",content:safeSectionContent("Important Information",focusPhrase)};const bookingIndex=draft.sections.findIndex(section=>/booking|cancellation|refund/i.test(String(section.heading||"")));draft.sections.splice(bookingIndex>=0?bookingIndex:draft.sections.length,0,timingSection)}
 timingSection.level="H2";timingSection.heading=focusPhrase+" Timings and Duration";
 const rank=section=>{const h=String(section.heading||"").toLowerCase();if(/related|similar|recommended/.test(h))return 95;if(/overview/.test(h))return 10;if(/duration option|booking option|rental period/.test(h))return 20;if(/what to expect/.test(h))return 30;if(/capacity|feature|ideal uses|experience|atmosphere|location|ticket information|historical|exhibition|dress code|safety|eligibility|redeem|arrival/.test(h))return 35;if(/highlight|inclusion/.test(h)&&!/exclusion/.test(h))return 40;if(/exclusion/.test(h))return 50;if(/timing|duration|schedule|service availability|operating hour/.test(h))return 60;if(/cancellation|refund|booking polic/.test(h))return 70;if(/important information|visitor guideline|safety guideline/.test(h))return 80;if(/why choose/.test(h))return 90;if(isFaqHeading(h))return 100;return 36};
 ...
 const relatedMatcher=section=>/related|similar|fleet options|recommended services|more experiences/i.test(String(section.heading||""));
 draft.sections=draft.sections.filter(section=>section===relatedSection||!relatedMatcher(section));
 const mergeSection=(target,source)=>{const parts=[target.content,source.content].map(value=>String(value||"").trim()).filter(Boolean);target.content=[...new Set(parts)].join("\n\n");for(const key of ["bullets","subsections","items"]){const combined=[...(Array.isArray(target[key])?target[key]:[]),...(Array.isArray(source[key])?source[key]:[])];if(combined.length){const seen=new Set();target[key]=combined.filter(item=>{const token=JSON.stringify(item);if(seen.has(token))return false;seen.add(token);return true})}}};
 const uniqueSections=[],seenH2=new Map();for(const section of draft.sections){const key=String(section.heading||"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim();if(key&&seenH2.has(key)){mergeSection(seenH2.get(key),section);continue}if(key)seenH2.set(key,section);uniqueSections.push(section)}draft.sections=uniqueSections;
 const canonicalRelatedIndex=draft.sections.indexOf(relatedSection);if(canonicalRelatedIndex>=0)draft.sections.splice(canonicalRelatedIndex,1);
 const finalWhyIndex=draft.sections.findIndex(section=>/why choose/i.test(String(section.heading||"")));draft.sections.splice(finalWhyIndex>=0?finalWhyIndex+1:draft.sections.length,0,relatedSection);
 const headingContains=phrase=>{const p=String(phrase||"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim();return draft.sections.some(section=>{const rows=[section,...(Array.isArray(section.subsections)?section.subsections:[]),...(Array.isArray(section.items)?section.items:[])];return rows.some(row=>String(row.heading||row.question||"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim().includes(p))})};
 for(const phrase of relatedPhrases){if(headingContains(phrase))continue;const tokens=String(phrase).toLowerCase(),target=/timing|hour|schedule/.test(tokens)?timingSection:/exclusion|charge/.test(tokens)?draft.sections.find(section=>/exclusion|additional charge/i.test(String(section.heading||""))):/inclusion|included/.test(tokens)?draft.sections.find(section=>/inclusion|included/i.test(String(section.heading||""))):/highlight/.test(tokens)?draft.sections.find(section=>/highlight/i.test(String(section.heading||""))):draft.sections.find(section=>/what to expect/i.test(String(section.heading||"")))||draft.sections[0];if(target){target.subsections=Array.isArray(target.subsections)?target.subsections:[];target.subsections.push({level:"H3",heading:phrase,content:"Details relevant to "+phrase+" are explained through the confirmed product information, selected booking option and applicable visitor guidance."})}}
 return draft;
}
...
function validateDraft(draft,category,baseline,yoastPlan){
 ...
 const validationResearch=draft?.research_contract||{},validationResearchSignals=Number(validationResearch.gsc_queries||0)+Number(validationResearch.google_suggestions||0)+Number(validationResearch.competitor_topics||0)+Number(validationResearch.first_party_catalog_topics||0)+Number(validationResearch.first_party_source_terms||0);if(validationResearch.completed!==true||validationResearchSignals<1)errors.push("Research contract is incomplete; use available Search Console, Google, competitor, first-party catalogue or protected source evidence before drafting");
