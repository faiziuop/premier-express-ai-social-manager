...
async function relatedProducts(job){
 ...
 const ranked=products.map(item=>{const itemTitle=decodeWp(item.title?.rendered),lower=itemTitle.toLowerCase(),itemTokens=new Set(researchTokens(lower)),overlap=tokens.filter(token=>itemTokens.has(token)).length,semanticScore=semantic.filter(term=>lower.includes(term)).length,intentScore=intent.filter(term=>lower.includes(term)).length;return{title:itemTitle,url:String(item.link||""),score:overlap*8+semanticScore*4+intentScore};}).filter(item=>item.url&&item.url.replace(/\/$/,"")!==own&&item.url.startsWith("https://dubaipremiertourism.com/product/")&&item.score>0).sort((a,b)=>b.score-a.score||a.title.localeCompare(b.title));
 return ranked.slice(0,6).map(({title,url})=>({text:title,url}));
}
...
function keywordHeadingErrors(draft,yoastPlan){
 const plan=yoastPlan||draft?.yoast_plan||{},sections=Array.isArray(draft?.sections)?draft.sections:[],headings=sections.flatMap(section=>[section.heading,...(Array.isArray(section.subsections)?section.subsections.map(item=>item?.heading):[]),...(Array.isArray(section.items)?section.items.map(item=>item?.question||item?.heading):[])]).map(value=>String(value||"")).join(" ").toLowerCase(),short=String(draft?.short_description||"").toLowerCase(),body=JSON.stringify(sections).toLowerCase();
 ...
 const relatedHeadingFamilies=related.filter(family=>includesFamily(headings,family)).length;
 if(related.length&&relatedHeadingFamilies<2)errors.push("Use at least two related-keyphrase families naturally across H2 or H3 headings");
 return errors;
}
function scoreDraftReadiness(draft,category,yoastPlan){
 ...
 const nested=sections.flatMap(section=>[...(Array.isArray(section.subsections)?section.subsections:[]),...(Array.isArray(section.items)?section.items:[])]),keywordErrors=keywordHeadingErrors(draft,plan);
 const contentChecks=[
  ...
  ["preservation",Array.isArray(draft?.preservation_ledger)&&draft.preservation_ledger.length>0,10]
 ];
 const seoChecks=[
  ["seo_title",String(plan.seo_title||"").length>=45&&String(plan.seo_title||"").length<=60,15],
  ["meta_description",String(plan.meta_description||"").length>=120&&String(plan.meta_description||"").length<=155,15],
  ["yoast_architecture",completeShadowKeywordPlan(plan),20],
  ["focus_short_description",!keywordErrors.some(error=>error.includes("short description")),15],
  ["focus_body",!keywordErrors.some(error=>error.endsWith("in the body")),10],
  ["focus_heading",!keywordErrors.some(error=>error.includes("focus keyphrase")),10],
  ["related_headings",!keywordErrors.some(error=>error.includes("related-keyphrase families")),10],
 ...
 return{content:summarize(contentChecks),seo:summarize(seoChecks),contract_version:"shadow-readiness-v2"};
}
function validateDraft(draft,category,baseline,yoastPlan){
 ...
 const inclusionSections=heads.filter(h=>(h.includes("highlight")||h.includes("inclusion"))&&!h.includes("exclusion")),exclusionSections=heads.filter(h=>h.includes("exclusion"));if(!inclusionSections.length||!exclusionSections.length)errors.push("Highlights and Inclusions and Exclusions must be separate H2 sections");
...
async function createShadowKeywordPlan(job,baseline,evidence,research,requestId){
 const existing=normalizeShadowKeywordPlan(baseline?.yoast_plan||{},job);if(completeShadowKeywordPlan(existing)&&baseline?.yoast_plan?.research_grounded===true&&Number(baseline?.yoast_plan?.research_contract_version||0)>=2)return{plan:baseline.yoast_plan,source:"PREVIOUS_RESEARCH_GROUNDED_SHADOW"};
 const instruction="Create a Yoast Premium 1+4 keyword plan before writing this Dubai tourism product. Return JSON only with seo_title (45-60 characters), meta_description (120-155 characters), focus_keyphrase, focus_synonyms (4-5), related_keyphrases (exactly 4 objects with keyphrase and exactly 4 synonyms). Use only product/category/source evidence; no invented claims or ranking promises. PRODUCT_AND_RESEARCH: "+JSON.stringify({title:job.product_title,category:job.blueprint_key,source:job.source_snapshot||{},assessment:job.assessment||{},evidence:evidence||[],search_research:research||{},rules:{prefer_relevant_first_party_gsc_queries:true,use_google_suggestions_as_language_signals:true,use_competitors_for_topic_gaps_only:true,never_copy_competitor_wording:true,never_treat_competitor_claims_as_product_facts:true}});
 try{const candidate=normalizeShadowKeywordPlan(await gateway([{role:"system",content:"Return only a valid JSON Yoast keyword plan."},{role:"user",content:instruction}]),job);candidate.seo_title=fitShadowSeoTitle(candidate.seo_title,candidate.focus_keyphrase);candidate.meta_description=fitShadowMeta(candidate.meta_description,candidate.focus_keyphrase);if(completeShadowKeywordPlan(candidate)){console.info("[shadow-generation]",{requestId,stage:"yoast_plan_created",source:"AI",focus:candidate.focus_keyphrase});candidate.research_grounded=true;candidate.research_contract_version=2;candidate.research_summary={gsc_queries:research?.gsc_queries?.length||0,google_suggestions:research?.google_suggestions?.length||0,competitor_topics:research?.competitor_topic_pages?.length||0};return{plan:candidate,source:"AI_RESEARCH_GROUNDED"}}}catch(error){console.warn("[shadow-generation]",{requestId,stage:"yoast_plan_fallback",message:error?.message||String(error)})}
 const fallback=deterministicKeywordPlan(job);fallback.seo_title=fitShadowSeoTitle(fallback.seo_title,fallback.focus_keyphrase);fallback.meta_description=fitShadowMeta(fallback.meta_description,fallback.focus_keyphrase);fallback.research_grounded=true;fallback.research_contract_version=2;fallback.research_summary={gsc_queries:research?.gsc_queries?.length||0,google_suggestions:research?.google_suggestions?.length||0,competitor_topics:research?.competitor_topic_pages?.length||0};console.info("[shadow-generation]",{requestId,stage:"yoast_plan_created",source:"DETERMINISTIC_FALLBACK",focus:fallback.focus_keyphrase});return{plan:fallback,source:"DETERMINISTIC_FALLBACK"}
}
function prompt(job,baseline,evidence,related,yoastPlan,research){return [
 ...
 "Short description 70-90 words; at least 450 useful body words across 10+ meaningful H2 sections; exactly 5 nonduplicate FAQs. Use the heading Frequently Asked Questions and store each FAQ in items as {level:\"H3\",question,answer}. Paragraphs for overview/expectation; bullets only where useful.",
 "Place Related Experiences immediately after Why Choose. Frequently Asked Questions follows Related Experiences. The backend will enforce this exact final-body order.",
 "YOAST PLAN CREATED BEFORE CONTENT: "+JSON.stringify(yoastPlan)+". Use its exact focus keyphrase, natural synonyms and four related keyphrases naturally across the short description, headings and body. At least one H2 or H3 must contain the focus keyphrase or a natural focus synonym, and related keyphrases may be used in other headings only where they read naturally. Do not stuff, force, list, or mechanically repeat keywords.",
 "Use varied Dubai tourism search phrases naturally, without stuffing. Avoid filler, repetition, research notes, internal workflow language, ranking claims, price superlatives, guarantees, world-class, ultimate, seamless, hassle-free, best and instant confirmation. If the model produces promotional excess, the backend will neutralize it without rejecting the complete draft.",
 ...
 "SEARCH RESEARCH: "+JSON.stringify(research||{})+". Search Console and Google suggestions may guide query language and intent. Competitor pages may guide topic gaps only; never copy their wording, links or operational claims.",
...
async function generateShadowDraft(req,res){
 ...
 const research=await shadowSearchResearch(job,auth,requestId);
 ...
 generated.yoast_plan=yoastPlan;generated.search_research_summary={created_at:research.created_at,gsc_queries:research.gsc_queries.length,google_suggestions:research.google_suggestions.length,competitor_topics:research.competitor_topic_pages.length};
