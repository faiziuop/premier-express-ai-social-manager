...
function researchTokens(value){return [...new Set(String(value||"").toLowerCase().replace(/&(?:amp|#038);/g," and ").replace(/[^a-z0-9\s]/g," ").split(/\s+/).filter(token=>token.length>2&&!["dubai","with","from","this","that","tour","service"].includes(token)))]}
async function shadowSearchResearch(job,auth,requestId){
 ...
 if(!gsc.length){const broad=await rest("search_console_metrics","GET",auth.authorization,{select:"query,page,clicks,impressions,ctr,position,dimension_date",order:"impressions.desc",limit:"500"}).catch(()=>[]),tokens=researchTokens(job.product_title);gsc=(Array.isArray(broad)?broad:[]).map(row=>({...row,relevance:tokens.filter(token=>String(row.query||"").toLowerCase().includes(token)).length})).filter(row=>row.relevance>0).sort((a,b)=>b.relevance-a.relevance||Number(b.impressions||0)-Number(a.impressions||0)).slice(0,40);}
...
function completeShadowKeywordPlan(value){const p=normalizeShadowKeywordPlan(value,{});return p.seo_title.length>=35&&p.meta_description.length>=100&&p.focus_keyphrase&&p.focus_synonyms.length>=4&&p.related_keyphrases.length===4&&p.related_keyphrases.every(x=>x.synonyms.length>=4)}
...
async function createShadowKeywordPlan(job,baseline,evidence,research,requestId){
 const existing=normalizeShadowKeywordPlan(baseline?.yoast_plan||{},job);if(completeShadowKeywordPlan(existing)&&baseline?.yoast_plan?.research_grounded===true)return{plan:baseline.yoast_plan,source:"PREVIOUS_RESEARCH_GROUNDED_SHADOW"};
