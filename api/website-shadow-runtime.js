...
function completeShadowKeywordPlan(value){const p=normalizeShadowKeywordPlan(value,{});return p.seo_title.length>=35&&p.meta_description.length>=100&&p.focus_keyphrase&&p.focus_synonyms.length>=3&&p.related_keyphrases.length===4&&p.related_keyphrases.every(x=>x.synonyms.length>=2)}
function deterministicKeywordPlan(job){
 ...
 const related=[["Dubai "+category,["Dubai "+category+" experience",category+" in Dubai","book Dubai "+category]],["Dubai tourism experience",["Dubai visitor experience","things to do in Dubai","Dubai travel activity"]],["book "+base,["reserve "+base,base+" booking","buy "+base+" tickets"]],["Dubai experience booking",["book Dubai attractions","Dubai activity booking","Dubai tour reservation"]]];
...
async function createShadowKeywordPlan(job,baseline,evidence,requestId){
 ...
 const instruction="Create a Yoast Premium 1+4 keyword plan before writing this Dubai tourism product. Return JSON only with seo_title (45-60 characters), meta_description (120-155 characters), focus_keyphrase, focus_synonyms (3-5), related_keyphrases (exactly 4 objects with keyphrase and 3-4 synonyms). Use only product/category/source evidence; no invented claims or ranking promises. PRODUCT: "+JSON.stringify({title:job.product_title,category:job.blueprint_key,source:job.source_snapshot||{},assessment:job.assessment||{},evidence:evidence||[]});
