...
async function createShadowKeywordPlan(job,baseline,evidence,research,requestId){
 const researchSignals=Number(research?.gsc_queries?.length||0)+Number(research?.google_suggestions?.length||0)+Number(research?.competitor_topic_pages?.length||0);
