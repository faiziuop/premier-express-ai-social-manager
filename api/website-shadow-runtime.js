...
async function createShadowKeywordPlan(job,baseline,evidence,requestId){
 const existing=normalizeShadowKeywordPlan(baseline?.yoast_plan||{},job);if(completeShadowKeywordPlan(existing))return{plan:existing,source:"PREVIOUS_SHADOW"};
 const instruction="Create a Yoast Premium 1+4 keyword plan before writing this Dubai tourism product. Return JSON only with seo_title (45-60 characters), meta_description (120-155 characters), focus_keyphrase, focus_synonyms (4-5), related_keyphrases (exactly 4 objects with keyphrase and exactly 4 synonyms). Use only product/category/source evidence; no invented claims or ranking promises. PRODUCT: "+JSON.stringify({title:job.product_title,category:job.blueprint_key,source:job.source_snapshot||{},assessment:job.assessment||{},evidence:evidence||[]});
 try{const candidate=normalizeShadowKeywordPlan(await gateway([{role:"system",content:"Return only a valid JSON Yoast keyword plan."},{role:"user",content:instruction}]),job);if(completeShadowKeywordPlan(candidate)){console.info("[shadow-generation]",{requestId,stage:"yoast_plan_created",source:"AI",focus:candidate.focus_keyphrase});return{plan:candidate,source:"AI"}}}catch(error){console.warn("[shadow-generation]",{requestId,stage:"yoast_plan_fallback",message:error?.message||String(error)})}
 const fallback=deterministicKeywordPlan(job);console.info("[shadow-generation]",{requestId,stage:"yoast_plan_created",source:"DETERMINISTIC_FALLBACK",focus:fallback.focus_keyphrase});return{plan:fallback,source:"DETERMINISTIC_FALLBACK"}
}
function prompt(job,baseline,evidence,related,yoastPlan){return [
